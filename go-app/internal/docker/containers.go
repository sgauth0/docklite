...
func (c *Client) ListContainers(ctx context.Context, all bool) ([]models.ContainerInfo, error) {
	...
	for _, item := range containers {
		...
		results = append(results, models.ContainerInfo{
			...
			Name:    name,
			Image:   item.Image,
			Created: fmt.Sprintf("%d", item.Created),
			State:   item.State,
			Status:  item.Status,
			Labels:  item.Labels,
		})
...
func (c *Client) CreateSiteContainer(ctx context.Context, domain string, templateType string, includeWww bool, sitePath string, port int) (string, error) {
	if templateType == "" {
		templateType = "static"
	}
	...
	labels := buildSiteLabels(domain, includeWww, templateType, internalPort)
...
func buildSiteLabels(domain string, includeWww bool, templateType string, internalPort int) map[string]string {
	sanitized := sanitizeDomain(domain)
