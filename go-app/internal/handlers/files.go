...
func (h *Handlers) listFiles(w http.ResponseWriter, r *http.Request) {
	...
	for i, entry := range entries {
		...
		relative, _ := filepath.Rel(filesBaseDir, filepath.Join(target, entry.Name()))
	...
	relativeTarget, _ := filepath.Rel(filesBaseDir, target)
	writeJSON(w, http.StatusOK, listFilesResponse{
		Path:    filepath.ToSlash(relativeTarget),
		Entries: results,
	})
}
